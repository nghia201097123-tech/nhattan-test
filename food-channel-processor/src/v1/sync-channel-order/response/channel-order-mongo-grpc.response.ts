import { UtilsBaseFunction } from "src/utils.common/utils.base-function.common/utils.base-function.common";
import { ChannelOrderMongoDetailGrpcResponse } from "./channel-food-mongo-detail-grpc.response";
import { UtilsDate } from "src/utils.common/utils.format-time.common/utils.format-time.common";

export class ChannelOrderGrpcResponse {
  ...
  constructor(entity?: any) {
    ...
    this.channel_order_food_name = UtilsBaseFunction.mapChannelOrderFoodName(this.channel_order_food_id);
    this.channel_order_food_code = UtilsBaseFunction.mapChannelOrderFoodCode(this.channel_order_food_id);
    this.details = new ChannelOrderMongoDetailGrpcResponse().mapToList(JSON.parse(entity?.details ?? "[]"),this.id,this.channel_order_food_id,this.order_id) ;
    ...
    this.create_at = UtilsDate.formatDateTimeVNToStringV2(entity?.createdAt ?? '');                
  ...
  public mapToList(data: any[]) {
    let response: ChannelOrderGrpcResponse[] = [];
    data.forEach((e) => {
      response.push(new ChannelOrderGrpcResponse(e));
    });
    return response;
  }
