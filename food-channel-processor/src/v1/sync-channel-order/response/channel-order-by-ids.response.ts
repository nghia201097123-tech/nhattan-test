...
export class ChannelOrderByIdsResponse {
  ...
  constructor(entity?: ChannelOrderByIdsDataModel) {
    this.id = +(entity?.id ?? 0);
  ...
  public mapToList(data: ChannelOrderByIdsDataModel[]) {
    let response: ChannelOrderByIdsResponse[] = [];
    data.forEach((e) => {
      response.push(new ChannelOrderByIdsResponse(e));
    });
    return response;
  }
