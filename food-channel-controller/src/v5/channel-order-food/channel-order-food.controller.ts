...
import { DataResponse } from "src/v4/grpc/interfaces/food-channel-validator";
import { BaseEmptyResponse } from "../grpc/interfaces/channel-order";
...
export class ChannelOrderFoodController {
  ...
  async getOrders(
    ...
    let keyGetChannelOrder = `food_channel_controller:get-channel-order-${user.restaurant_id}-${restaurantBrandId}-${branchId}-${channelOrderFoodId}-${customerOrderStatus}-${isHaveRestaurantOrder}-${isAppFood}-${page}-${limit}-v5`;  
    ...
    if (!listChannelOrders) {
      ...
      let dataFoodChannelValidator : DataResponse = null ; 

      const keyFoodChannelValidator = await this.redisService.getKey(
        `food_channel:validator-${user.restaurant_id}-${restaurantBrandId}-${branchId}`
      );
      
      if(!keyFoodChannelValidator){
        ...
        if (dataToCheckRedis.status != HttpStatus.OK) {
          ...
          response.setData({
            errors: [],
            list: [],
            is_new_order_app_food : 0,
            is_new_order_online : 0 
          });
          return res.status(HttpStatus.OK).send(response);
        }    

        dataFoodChannelValidator = dataToCheckRedis.data;
        
      }else{
        
        dataFoodChannelValidator = JSON.parse(keyFoodChannelValidator);

      }
      
      let dataOrderGrpc = await this.channelOrderFoodService.getOrdersGrpcV4(
        user.restaurant_id,
        restaurantBrandId,
        branchId,
        channelOrderFoodId,
        ...
        isAppFood,
        dataFoodChannelValidator?.is_active ?? 0,
        JSON.stringify(dataFoodChannelValidator?.tokens ?? []),
        dataFoodChannelValidator?.is_use_kafka ?? 0
      )
      ...
      response.setData({
        ...
        is_new_order_app_food : dataOrderGrpc.data?.is_new_order_app_food ?? 0,
        is_new_order_online : dataOrderGrpc.data?.is_new_order_online ?? 0 
      })
    ...
    console.log(`Log time v5 : ${user.restaurant_id} - ${restaurantBrandId} - ${branchId} - begin( ${timeBegin} ) - end( ${new Date()} )`);
  ...
  async getDetail(
    ...
    let keyGetChannelOrder = `food_channel_controller:get-order-detail-${user.restaurant_id}-${id}-${isAppFood}-v5`;  

    let channelOrder = await this.redisService.getKey(
      keyGetChannelOrder
    );

    if (!channelOrder) {

      let dataGrpc = await this.channelOrderFoodService.getOrderDetailGrpcV2(
        id,
        isAppFood,
        user.restaurant_id
      );
      ...
      await this.redisService.setKeyV3(keyGetChannelOrder,JSON.stringify(dataGrpc.data),5);

      response.setData(new CustomerOrderResponse(dataGrpc.data));
    }else{
      response.setData(new CustomerOrderResponse(JSON.parse(channelOrder)));
    }
  ...
  async updateOrderHandleCompleteV2(
    ...
    let dataGrpc = await this.channelOrderFoodService.handleCompleteChannelOrderV3Grpc(
      user.restaurant_id,
      dto.id,
  ...
  async updateOrderHandleCancelV2(
    ...
    let dataGrpc = await this.channelOrderFoodService.handleCancelChannelOrderV3Grpc(
      user.restaurant_id,
      dto.id,
      dto.restaurant_order_id
    );
  ...
  async batchConfirmChannelOrder(
    ...
    let data : BaseEmptyResponse = await this.channelOrderFoodService.batchConfirmChannelOrderGrpcV3(
        user.restaurant_id,
        dto.branch_id,
  ...
  async updateCancelPrintOrders(
    ...
    let dataGrpc = await this.channelOrderFoodService.updateCancelPrintOrdersGrpcV2(
        user.restaurant_id,
        dto.branch_id,
        JSON.stringify(dto.ids)
      );
  ...
  async getDetailToPrint(
    ...
    let keyGetChannelOrder = `food_channel_controller:get-print-order-detail-${user.restaurant_id}-${id}-v5`;  
    ...
    if (!channelOrder) {
      ...
      let dataGrpc = await this.channelOrderFoodService.getOrderDetailToPrintGrpcV2(
        id,
        user.restaurant_id
      );
      ...
      await this.redisService.setKeyV3(keyGetChannelOrder,JSON.stringify(dataGrpc.data),10);
