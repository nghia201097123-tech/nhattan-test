...
@Controller({
  version: VersionEnum.V4.toString(),
...
export class ChannelOrderFoodController {
  ...
  async getOrders(
    ...
    let keyGetChannelOrder = `food_channel_controller:get-channel-order-${user.restaurant_id}-${restaurantBrandId}-${branchId}-${channelOrderFoodId}-${customerOrderStatus}-${isHaveRestaurantOrder}-${isAppFood}-${page}-${limit}-v4`;  
    ...
    if (!listChannelOrders) {
      ...
      let dataOrderGrpc = await this.channelOrderFoodService.getOrdersGrpcV3(
        user.restaurant_id,
        restaurantBrandId,
        branchId,
        channelOrderFoodId,
    ...
    console.log(`Log time v4 : ${user.restaurant_id} - ${restaurantBrandId} - ${branchId} - begin( ${timeBegin} ) - end( ${new Date()} )`);
  ...
  async getDetail(
    ...
    let keyGetChannelOrder = `food_channel_controller:get-order-detail-${user.restaurant_id}-${id}-${isAppFood}-v4`;  
  ...
  async batchConfirmChannelOrder(
    ...
    let dataGrpc = await this.channelOrderFoodService.batchConfirmChannelOrderGrpcV2(
      user.restaurant_id,
      dto.branch_id,
  ...
  async getDetailToPrint(
    ...
    let keyGetChannelOrder = `food_channel_controller:get-print-order-detail-${user.restaurant_id}-${id}-v4`;  
