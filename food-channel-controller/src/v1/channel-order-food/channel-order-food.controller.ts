...
export class ChannelOrderFoodController {
  ...
  async getOrders(
    ...
    let dataOrderGrpc = await this.channelOrderFoodService.getOrdersGrpc(
      user.restaurant_id,
      restaurantBrandId,
      branchId,
      channelOrderFoodId,
    ...
    response.setData({
      ...
      list: new CustomerOrderResponse().mapToList(!dataOrderGrpc.data.list ? [] : dataOrderGrpc.data.list)
    })

    console.log(`Log time : ${user.restaurant_id} - ${restaurantBrandId} - ${branchId} - begin( ${timeBegin} ) - end( ${new Date()} )`);
